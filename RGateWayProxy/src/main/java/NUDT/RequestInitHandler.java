...
import NUDT.handler.Dest2ClientHandler;
...
//            ServerConfigLoader serverConfigLoader = new ServerConfigLoader(new JsonConfigReader<>());
//            serverConfigLoader.loadConfig("server_config.json",ServerConfig.class);


            //ChannelFuture future = bootstrap.connect(ServerConfig.INSTANCE.getServerHost(),ServerConfig.INSTANCE.getServerPort());
...
//                        clientChannelContext.writeAndFlush(buf);
                        ...
                        //
//                        final ByteBuf buf1 = Unpooled.copiedBuffer(addr + ":" + port + "zjp",CharsetUtil.UTF_8);
//                        LOG.debug(buf1.toString(CharsetUtil.UTF_8));
//                        future.channel().writeAndFlush(buf1);


                    } else {
//                        final ByteBuf buf = Unpooled.copiedBuffer("1111",
//                                CharsetUtil.UTF_8);
                        clientChannelContext.writeAndFlush(Unpooled.EMPTY_BUFFER);
                    }
                }
            });
        }
