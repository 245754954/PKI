package NUDT.tsa.client.impl;

import NUDT.ex.LittleTsaException;
import NUDT.tsa.TsaTranserConstant;
import NUDT.tsa.client.TsaClient;
...
public class LittleTsaClientImpl implements TsaClient {
    ...
    public TimeStampResponse timestamp(String signAlg, byte[] messageDigest) {
        ...
        try {
            ...
            MediaType mediaType = MediaType.parse("application/timestamp-query; charset=utf-8");
            Request timeStampOkHttpRequest = new Request.Builder().addHeader("Content-Type", TsaTranserConstant.TIMESTAMP_QUERY_CONTENT_TYPE)
                    .addHeader(TsaTranserConstant.TRANSFER_ENCODING_BASE64, dataType).url(tsaUrl)
